...
begin
  if coalesce(auth.role(), '') <> 'service_role' then
    raise exception 'service-role broker required' using errcode = '42501';
  end if;
  if p_actor_user_id is null or not exists (
    select 1 from auth.users caller
    where caller.id = p_actor_user_id
      and coalesce(caller.is_anonymous, false) = false
  ) then
    raise exception 'existing non-anonymous administrator required' using errcode = '22023';
  end if;
  if p_organization_id is null or p_target_user_id is null then
    raise exception 'organization and target user are required' using errcode = '22023';
  end if;
  if p_role is null and p_status is null then
    raise exception 'role or status change required' using errcode = '22023';
  end if;
  if p_target_user_id = p_actor_user_id then
    raise exception 'cannot change your own membership through the user-admin workflow' using errcode = '42501';
  end if;
  ...
  if actor_role is null or actor_role not in (
    ...
    raise exception 'active owner or administrator membership required' using errcode = '42501';
  end if;
  ...
  if next_status = 'invited'::public.membership_status then
    raise exception 'use invitation workflow for invited membership' using errcode = '22023';
  end if;

  if actor_role = 'admin'::public.member_role then
    if current_membership.role in ('owner'::public.member_role,'admin'::public.member_role) then
      raise exception 'administrators cannot modify owner or admin memberships' using errcode = '42501';
    end if;
    if next_role in ('owner'::public.member_role,'admin'::public.member_role) then
      raise exception 'administrators cannot grant owner or admin roles' using errcode = '42501';
    end if;
  end if;
  ...
  set role = next_role,
      status = next_status,
      joined_at = case when next_status = 'active'::public.membership_status
        then coalesce(joined_at, changed_at) else joined_at end,
      updated_at = changed_at
  where organization_id = p_organization_id
    and user_id = p_target_user_id;

  event_type := case
    when next_status = 'revoked'::public.membership_status then 'organization.member.revoked'
    when next_status = 'suspended'::public.membership_status then 'organization.member.suspended'
    when current_membership.status <> 'active'::public.membership_status
      and next_status = 'active'::public.membership_status then 'organization.member.activated'
    when current_membership.role <> next_role then 'organization.member.role_changed'
    else 'organization.member.updated'
  end;

  perform private.append_audit_event(
    p_organization_id,null,null,'human'::public.audit_actor_type,p_actor_user_id,event_type,
    jsonb_build_object(
...
revoke all on function public.pandora_admin_update_organization_member(
  uuid,uuid,uuid,public.member_role,public.membership_status
