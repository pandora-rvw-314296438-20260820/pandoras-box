-- Pandora Visible Creation — History compaction metrics over canonical conversation truth
...
begin
  ...
  build_items as (
    select
      ...
      r.id,
      j.id,
      coalesce(j.target_project_version_id, v.id),
      ...
      jsonb_strip_nulls(jsonb_build_object(
        ...
        'buildNumber', v.sequence_no,
        'durationMs', case
          when j.completed_at is not null then
            greatest(0, (extract(epoch from (j.completed_at - coalesce(j.started_at, j.created_at))) * 1000)::bigint)
          else null
        end,
        'fileCount', v.source_payload -> 'fileCount',
        'lineCount', v.source_payload -> 'lineCount',
    ...
    left join lateral (
      select
        ...
        pv.sequence_no,
        pv.source_payload,
        pv.verification_run_id
      from public.pandora_project_versions pv
      where pv.organization_id = j.organization_id
        and pv.project_id = j.project_id
        and pv.build_job_id = j.id
      order by pv.sequence_no desc
      limit 1
    ) v on true
    left join lateral (
      select
      ...
      where vc.organization_id = j.organization_id
        and vc.project_id = j.project_id
        and vc.verification_run_id = v.verification_run_id
    ) c on true
