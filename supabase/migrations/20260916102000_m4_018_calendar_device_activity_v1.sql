...
-- M4-018 calendar/reminder canonical Activity bridge v1.
...
begin
  ...
  if v_capability not in ('calendar.events','reminder.local') then
  ...
  case v_stage
    ...
    when 'failed' then
      ...
      v_message := case v_capability
        ...
        else 'Android local reminder action failed or could not be verified.'
      end;
    when 'needs_permission' then
      ...
      v_blocker := jsonb_build_object(
        ...
        'requiredAction',case v_capability
          ...
          else 'Grant Notifications permission to Pandora in Android app permissions.'
        end,
    ...
    when 'needs_choice' then
      v_state := 'needs_you';
      v_message := 'Pandora needs one calendar choice before making a consequential change.';
      v_blocker := jsonb_build_object(
        'reasonCode','missing_consequential_user_choice',
        'reason','More than one safe calendar target matches the request.',
        'requiredAction','Choose the exact calendar or event to change.',
        'approvalRequired',false,
        'policyRef','m4-018-calendar-target-disambiguation'
      );
  end case;
  ...
  v_event := jsonb_build_object(
    ...
    'provenance',jsonb_build_object(
      'sourceType','device',
      'sourceId','pandora-android-calendar-v1',
...
is 'M4-018 bounded authenticated calendar/reminder device-fact ingress into canonical Activity Theatre/History.';
