...
declare
  ...
  v_replay_version public.pandora_project_versions%rowtype;
  v_replay_artifact public.pandora_artifact_versions%rowtype;
begin
  ...
  if v_job.target_project_version_id is not null then
    select * into v_replay_version
    from public.pandora_project_versions pv
    where pv.id=v_job.target_project_version_id
      and pv.organization_id=p_organization_id
      and pv.project_id=p_project_id
      and pv.project_spec_id=p_project_spec_id
      and pv.build_job_id=v_job.id
      and pv.created_by is not distinct from p_requested_by
    for share;
    if not found then
      raise exception 'BUILD_INTAKE_REPLAY_MISMATCH' using errcode='23514';
    end if;

    select * into v_replay_artifact
    from public.pandora_artifact_versions av
    where av.id=v_replay_version.root_artifact_version_id
      and av.organization_id=p_organization_id
      and av.project_id=p_project_id
    for share;
    if not found
      or v_replay_version.source_sha256 is distinct from p_source_sha256
      or v_replay_artifact.content_sha256 is distinct from p_source_sha256
      or v_replay_artifact.byte_size is distinct from p_source_byte_size
      or v_replay_artifact.storage_path is distinct from p_storage_path
      or v_replay_version.source_payload->>'buildAdapter' is distinct from p_build_adapter
      or not exists(
        select 1 from public.pandora_model_runs mr
        where mr.id=p_model_run_id
          and mr.organization_id=p_organization_id
          and mr.project_id=p_project_id
          and mr.project_spec_id=p_project_spec_id
          and mr.build_job_id=v_job.id
