...
create or replace function public.pandora_register_project(
...
begin
  ...
  insert into public.pandora_projects (
    ...
    'pandora/projects/' || v_key, coalesce(p_objective, ''), p_created_by
  )
  on conflict (organization_id, project_key) do update set
    name = excluded.name,
    repository = coalesce(excluded.repository, pandora_projects.repository),
    objective = case when excluded.objective <> '' then excluded.objective else pandora_projects.objective end
  ...
  foreach v_document_key in array array[
    ...
    insert into public.pandora_workspace_documents (
...
create or replace function public.pandora_accept_intake(
  p_organization_id uuid,
  p_requester_id uuid,
  p_request_text text,
...
declare
  ...
  v_key text;
  v_policy public.pandora_policies%rowtype;
begin
  ...
  if p_requester_id is not null and auth.role() <> 'service_role' and p_requester_id <> auth.uid() then
    raise exception 'pandora_requester_mismatch';
  end if;

  select * into v_policy from public.pandora_policies where organization_id = p_organization_id;
  if found and v_policy.allow_untracked_work then raise exception 'invalid_pandora_policy'; end if;
  ...
  if not found then
    perform public.pandora_register_project(
      ...
      p_repository,
      'Automatically created by mandatory Pandora intake.',
      p_requester_id
    );
  ...
  insert into public.pandora_intake_requests (
...
create or replace function public.pandora_record_external_event(
  p_organization_id uuid,
  p_provider text,
  p_delivery_id text,
  p_event_type text,
...
declare
  v_project_id uuid;
  v_event public.pandora_external_events%rowtype;
begin
  perform private.assert_control_service_role();
  select id into v_project_id from public.pandora_projects
   where organization_id = p_organization_id
  ...
  insert into public.pandora_external_events (
  ...
  on conflict (organization_id, provider, delivery_id) do update set
    project_id = coalesce(pandora_external_events.project_id, excluded.project_id),
    process_status = 'processed', processed_at = now()
  returning * into v_event;

  if v_project_id is not null then
    insert into public.pandora_integration_health (
...
create or replace function public.pandora_recompute_project(
  p_organization_id uuid,
  p_project_key text
) returns jsonb
language plpgsql
...
declare
  v_project public.pandora_projects%rowtype;
  v_task public.pandora_tasks%rowtype;
  v_missing integer;
  v_criteria integer;
  v_satisfied integer;
  ...
  v_next_task public.pandora_tasks%rowtype;
  v_current_phase public.pandora_phases%rowtype;
  v_projection jsonb;
  v_version bigint;
begin
  if auth.role() <> 'service_role' and not private.is_org_member(p_organization_id) then
  ...
  for v_task in select * from public.pandora_tasks where project_id = v_project.id order by sequence loop
    select count(*) into v_missing
      from public.pandora_task_dependencies d
      join public.pandora_tasks required_task on required_task.id = d.depends_on_task_id
    ...
    if v_criteria > 0 then
      ...
      select count(*) into v_satisfied from public.pandora_evidence e
    ...
    update public.pandora_tasks set
      status = case
  ...
  update public.pandora_phases phase set
    status = case
      when not exists (select 1 from public.pandora_tasks t where t.phase_id = phase.id and t.blocks_phase_exit and t.status <> 'complete') then 'complete'
      when exists (select 1 from public.pandora_tasks t where t.phase_id = phase.id and t.status = 'blocked') then 'blocked'
      when exists (select 1 from public.pandora_tasks t where t.phase_id = phase.id and t.status in ('ready','in_progress','waiting_review','waiting_approval')) then 'active'
      else 'planned'
    end,
    started_at = case when exists (select 1 from public.pandora_tasks t where t.phase_id = phase.id and t.status in ('ready','in_progress','waiting_review','waiting_approval','complete')) then coalesce(phase.started_at, now()) else phase.started_at end,
    completed_at = case when not exists (select 1 from public.pandora_tasks t where t.phase_id = phase.id and t.blocks_phase_exit and t.status <> 'complete') then coalesce(phase.completed_at, now()) else null end
  where phase.project_id = v_project.id;

  select * into v_next_task from public.pandora_tasks t
  ...
  if v_next_task.phase_id is not null then select * into v_current_phase from public.pandora_phases where id = v_next_task.phase_id; end if;
  ...
  from public.pandora_tasks where project_id = v_project.id and status <> 'cancelled';

  update public.pandora_projects set
    ...
    status = case
      ...
      when exists (select 1 from public.pandora_tasks where project_id = v_project.id and status = 'blocked') and v_next_task.id is null then 'blocked'
      else 'active'
    end,
  ...
  select jsonb_build_object(
    ...
    'phases', coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'completedTasks', (select count(*) from public.pandora_tasks t where t.phase_id = p.id and t.status = 'complete'),
      'gatingTasks', (select count(*) from public.pandora_tasks t where t.phase_id = p.id and t.blocks_phase_exit)
    ) order by p.sequence) from public.pandora_phases p where p.project_id = v_project.id), '[]'::jsonb),
    'tasks', coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'missingDependencies', coalesce((select jsonb_agg(dep.task_key) from public.pandora_task_dependencies d join public.pandora_tasks dep on dep.id=d.depends_on_task_id where d.task_id=t.id and dep.status <> 'complete'), '[]'::jsonb),
      'evidence', coalesce((select jsonb_agg(jsonb_build_object('type',e.evidence_type,'provider',e.provider,'status',e.status,'headSha',e.head_sha,'url',e.source_url,'observedAt',e.observed_at) order by e.observed_at desc) from public.pandora_evidence e where e.task_id=t.id and e.invalidated_at is null), '[]'::jsonb)
    ) order by t.sequence) from public.pandora_tasks t where t.project_id = v_project.id), '[]'::jsonb),
    'integrations', coalesce((select jsonb_agg(jsonb_build_object('provider',h.provider,'status',h.status,'lastEventAt',h.last_event_at,'lastSuccessAt',h.last_success_at,'staleAfter',h.stale_after,'details',h.details)) from public.pandora_integration_health h where h.project_id=v_project.id), '[]'::jsonb),
    'lessons', coalesce((select jsonb_agg(jsonb_build_object('scope',l.scope,'category',l.category,'lesson',l.lesson,'confidence',l.confidence,'evidenceCount',l.evidence_count,'status',l.status) order by l.confidence desc,l.evidence_count desc) from public.pandora_lessons l where l.organization_id=p_organization_id and (l.project_id=v_project.id or l.scope in ('portfolio','operational')) and l.status in ('candidate','promoted')), '[]'::jsonb),
    'liveSync', jsonb_build_object('mode','durable','healthy',true,'lastReconciledAt',v_project.last_reconciled_at,'stale',false)
  ) into v_projection;

  insert into public.pandora_projections (organization_id, project_id, version, projection, computed_at, stale_after)
  values (p_organization_id, v_project.id, 1, v_projection, now(), now() + interval '15 minutes')
  on conflict (project_id) do update set
    version = pandora_projections.version + 1,
  ...
  update public.pandora_projections set projection = v_projection where project_id = v_project.id;

  insert into public.pandora_workspace_documents (organization_id, project_id, document_key, content)
  values
  ...
  on conflict (project_id, document_key) do update set
    version = pandora_workspace_documents.version + 1,
...
create or replace function public.pandora_record_evidence(
  p_organization_id uuid,
  p_project_key text,
  p_task_key text,
...
declare
  v_project public.pandora_projects%rowtype;
  v_task public.pandora_tasks%rowtype;
  v_evidence public.pandora_evidence%rowtype;
  v_projection jsonb;
begin
  perform private.assert_control_service_role();
  select * into strict v_project from public.pandora_projects where organization_id=p_organization_id and project_key=p_project_key;
  select * into strict v_task from public.pandora_tasks where project_id=v_project.id and task_key=p_task_key;

  if p_head_sha is not null then
    update public.pandora_evidence set
    ...
    update public.pandora_tasks set current_head_sha=p_head_sha where id=v_task.id;
  end if;
...
create or replace function public.pandora_record_outcome(
  p_organization_id uuid,
  p_project_key text,
  p_task_key text,
  p_success boolean,
...
declare
  ...
  v_task public.pandora_tasks%rowtype;
  v_item jsonb;
  v_existing public.pandora_lessons%rowtype;
...
begin
  ...
  select * into strict v_project from public.pandora_projects where organization_id=p_organization_id and project_key=p_project_key;
  select * into strict v_task from public.pandora_tasks where project_id=v_project.id and task_key=p_task_key;
  select promotion_min_evidence into v_threshold from public.pandora_policies where organization_id=p_organization_id;
  v_threshold := coalesce(v_threshold,3);

  insert into public.pandora_task_outcomes (
  ...
  for v_item in select value from jsonb_array_elements(coalesce(p_agent_observations,'[]'::jsonb)) loop
    insert into public.pandora_agent_observations (
  ...
  for v_item in select value from jsonb_array_elements(coalesce(p_lessons,'[]'::jsonb)) loop
    ...
    select * into v_existing from public.pandora_lessons
    ...
    if found then
      update public.pandora_lessons set
      ...
      insert into public.pandora_lessons (
  ...
  insert into public.pandora_learning_runs (organization_id,project_id,status,inputs,outputs,completed_at)
  ...
  return jsonb_build_object('ok',true,'projection',public.pandora_recompute_project(p_organization_id,p_project_key));
end;
$$;

create or replace function public.pandora_get_dashboard(
  p_organization_id uuid,
  p_project_key text default null
) returns jsonb
language plpgsql
...
declare
  ...
  v_project record;
begin
  if auth.role() <> 'service_role' and not private.is_org_member(p_organization_id) then raise exception 'pandora_forbidden'; end if;
  if p_project_key is not null then
    select projection into v_result from public.pandora_projections pr
      join public.pandora_projects p on p.id=pr.project_id
      where p.organization_id=p_organization_id and p.project_key=p_project_key;
    if v_result is null then v_result := public.pandora_recompute_project(p_organization_id,p_project_key); end if;
    return v_result;
  end if;

  for v_project in select project_key from public.pandora_projects where organization_id=p_organization_id and status <> 'archived' loop
    perform public.pandora_recompute_project(p_organization_id,v_project.project_key);
  end loop;

  select jsonb_build_object(
    'schemaVersion','2.0.0','observedThrough',now(),
    'policy',(select to_jsonb(p) from public.pandora_policies p where p.organization_id=p_organization_id),
  ...
  from public.pandora_projects p left join public.pandora_projections pr on pr.project_id=p.id
  where p.organization_id=p_organization_id and p.status <> 'archived';
  return v_result;
end;
$$;

grant execute on function public.pandora_register_project(uuid,text,text,text,text,uuid) to authenticated, service_role;
grant execute on function public.pandora_accept_intake(uuid,uuid,text,text,text,text,text,text,text) to authenticated, service_role;
grant execute on function public.pandora_record_external_event(uuid,text,text,text,text,jsonb,text,text,timestamptz) to service_role;
grant execute on function public.pandora_record_evidence(uuid,text,text,text,text,text,text,text,text,text,text,jsonb) to service_role;
grant execute on function public.pandora_recompute_project(uuid,text) to authenticated, service_role;
grant execute on function public.pandora_record_outcome(uuid,text,text,boolean,jsonb,jsonb,numeric,integer,integer,text,jsonb,jsonb,jsonb) to service_role;
grant execute on function public.pandora_get_dashboard(uuid,text) to authenticated, service_role;

insert into public.pandora_policies (organization_id, policy)
values ('2270b266-59da-4c39-bfd9-9f8d08352af0', jsonb_build_object(
  'workflow','owner_request -> pandora_intake -> workspace -> plan -> execution -> independent_review -> evidence -> merge_deploy -> outcome -> learning',
  'phoneOnly',true,
...
select public.pandora_register_project('2270b266-59da-4c39-bfd9-9f8d08352af0','mcpmaster','MCPMaster / Pandora','mbanatao/mcpmaster','Mandatory Banatao Systems control layer for all project work.','76a3bf0a-5bfa-4ce2-b92a-3646824e5754');
select public.pandora_register_project('2270b266-59da-4c39-bfd9-9f8d08352af0','callgate','CallGate','mbanatao/callgate-platform','Production-grade branded calling platform and proving project for Pandora.','76a3bf0a-5bfa-4ce2-b92a-3646824e5754');
select public.pandora_register_project('2270b266-59da-4c39-bfd9-9f8d08352af0','battle','Battle','mbanatao/Battle','Legal operations and client-engagement system.','76a3bf0a-5bfa-4ce2-b92a-3646824e5754');
select public.pandora_register_project('2270b266-59da-4c39-bfd9-9f8d08352af0','realmatch','RealMatch','mbanatao/realmatch','Matching, qualification, trust, and conversion infrastructure.','76a3bf0a-5bfa-4ce2-b92a-3646824e5754');
select public.pandora_register_project('2270b266-59da-4c39-bfd9-9f8d08352af0','memory','Memory','mbanatao/Memory','AI memory and intelligence infrastructure.','76a3bf0a-5bfa-4ce2-b92a-3646824e5754');

insert into public.pandora_phases (organization_id,project_id,phase_key,name,sequence)
select p.organization_id,p.id,'kernel','Mandatory operating kernel',0 from public.pandora_projects p where p.project_key='mcpmaster'
on conflict (project_id,phase_key) do nothing;
insert into public.pandora_phases (organization_id,project_id,phase_key,name,sequence)
select p.organization_id,p.id,'production-proof','CallGate production proof',0 from public.pandora_projects p where p.project_key='callgate'
on conflict (project_id,phase_key) do nothing;

insert into public.pandora_tasks (organization_id,project_id,phase_id,task_key,title,description,sequence,priority,status,continuation_role,blocks_phase_exit,risk_class,builder_agent,builder_vendor,reviewer_agent,reviewer_vendor,completion_criteria,current_head_sha,current_pr_number)
select p.organization_id,p.id,ph.id,'POS-KERNEL-001','Implement durable Pandora operating kernel','Durable workspaces, mandatory intake, events, evidence, projections, outcomes, lessons, and learning.',0,100,'in_progress','primary',true,'R2','A-CHATGPT','OpenAI','Vercel Agent','Vercel',
...
from public.pandora_projects p join public.pandora_phases ph on ph.project_id=p.id and ph.phase_key='kernel' where p.project_key='mcpmaster'
...
insert into public.pandora_tasks (organization_id,project_id,phase_id,task_key,title,description,sequence,priority,continuation_role,blocks_phase_exit,risk_class,builder_agent,builder_vendor,reviewer_agent,reviewer_vendor,completion_criteria)
select p.organization_id,p.id,ph.id,'POS-KERNEL-002','Enforce mandatory Pandora intake','Every request resolves to or creates a durable project workspace before execution.',1,95,'primary',true,'R2','A-CHATGPT','OpenAI','A-COPILOT','GitHub','["database_migration_applied","intake_api_verified"]'::jsonb
from public.pandora_projects p join public.pandora_phases ph on ph.project_id=p.id and ph.phase_key='kernel' where p.project_key='mcpmaster'
...
insert into public.pandora_task_dependencies (organization_id,project_id,task_id,depends_on_task_id)
select t.organization_id,t.project_id,t.id,d.id from public.pandora_tasks t join public.pandora_tasks d on d.project_id=t.project_id
...
from public.pandora_projects p join public.pandora_phases ph on ph.project_id=p.id and ph.phase_key='production-proof' where p.project_key='callgate'
on conflict (project_id,task_key) do update set completion_criteria=excluded.completion_criteria,current_head_sha=excluded.current_head_sha,current_pr_number=excluded.current_pr_number;
insert into public.pandora_tasks (organization_id,project_id,phase_id,task_key,title,description,sequence,priority,continuation_role,blocks_phase_exit,risk_class,builder_agent,builder_vendor,reviewer_agent,reviewer_vendor,completion_criteria)
...
with projects as (
  select * from public.pandora_projects where project_key in ('battle','realmatch','memory')
), phases as (
  insert into public.pandora_phases (organization_id,project_id,phase_key,name,sequence)
...
select public.pandora_record_evidence('2270b266-59da-4c39-bfd9-9f8d08352af0','callgate','PO-015','github_issue_closed','github','callgate-issue-112','complete','https://github.com/mbanatao/callgate-platform/issues/112','mbanatao/callgate-platform',null,'pass',jsonb_build_object('issue',112));
select public.pandora_record_evidence('2270b266-59da-4c39-bfd9-9f8d08352af0','callgate','PO-015','github_pr_merged','github','callgate-pr-114-merge','complete','https://github.com/mbanatao/callgate-platform/pull/114','mbanatao/callgate-platform','699542f18ccf08ce299c7a70225728507959c96e','pass',jsonb_build_object('mergeCommit','febbf1f1743ed8a25b194005df8fe7bc5de68dc7'));
select public.pandora_record_evidence('2270b266-59da-4c39-bfd9-9f8d08352af0','callgate','PO-015','github_ci_success','github','callgate-run-30615099412','passing','https://github.com/mbanatao/callgate-platform/actions/runs/30615099412','mbanatao/callgate-platform','699542f18ccf08ce299c7a70225728507959c96e','pass',jsonb_build_object('runId',30615099412));
select public.pandora_record_evidence('2270b266-59da-4c39-bfd9-9f8d08352af0','callgate','PO-015','independent_review_pass','github','callgate-pr-114-review','passing','https://github.com/mbanatao/callgate-platform/pull/114','mbanatao/callgate-platform','699542f18ccf08ce299c7a70225728507959c96e','pass','{}'::jsonb);

select public.pandora_recompute_project('2270b266-59da-4c39-bfd9-9f8d08352af0','mcpmaster');
select public.pandora_recompute_project('2270b266-59da-4c39-bfd9-9f8d08352af0','callgate');
select public.pandora_recompute_project('2270b266-59da-4c39-bfd9-9f8d08352af0','battle');
select public.pandora_recompute_project('2270b266-59da-4c39-bfd9-9f8d08352af0','realmatch');
select public.pandora_recompute_project('2270b266-59da-4c39-bfd9-9f8d08352af0','memory');
