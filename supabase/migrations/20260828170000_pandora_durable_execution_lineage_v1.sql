...
create table if not exists public.pandora_build_job_events (
  id bigint generated always as identity primary key, organization_id uuid not null references public.organizations(id) on delete cascade, project_id uuid not null references public.pandora_projects(id) on delete cascade, build_job_id uuid not null references public.pandora_build_jobs(id) on delete cascade, event_type text not null, from_status text null, to_status text null, safe_payload jsonb not null default '{}'::jsonb, actor_type text not null default 'system', actor_id text null, request_id text null, idempotency_key text null, created_at timestamptz not null default now(),
...
create table if not exists public.pandora_model_runs (
  id uuid primary key default gen_random_uuid(),organization_id uuid not null references public.organizations(id) on delete cascade,project_id uuid not null references public.pandora_projects(id) on delete cascade,project_spec_id uuid not null references public.pandora_project_specs(id) on delete restrict,build_job_id uuid null references public.pandora_build_jobs(id) on delete set null,build_job_step_id uuid null references public.pandora_build_job_steps(id) on delete set null,request_id text not null,task text not null,output_mode text not null default 'structured',status text not null default 'queued',provider text null,model text null,model_revision text null,request_sha256 text not null,context_sha256 text null,schema_sha256 text null,response_sha256 text null,input_tokens bigint not null default 0,output_tokens bigint not null default 0,total_tokens bigint not null default 0,estimated_cost_micros bigint not null default 0,billed_cost_micros bigint not null default 0,attempt integer not null default 1,max_attempts integer not null default 1,error_code text null,error_public_summary text null,started_at timestamptz null,completed_at timestamptz null,created_at timestamptz not null default now(),
