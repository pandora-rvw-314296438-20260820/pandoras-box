...
create or replace function public.pandora_set_integration_health(
...
declare
  v_project_id uuid;
  v_row public.pandora_integration_health%rowtype;
begin
  ...
  insert into public.pandora_integration_health(
    ...
    last_event_at=coalesce(excluded.last_event_at,pandora_integration_health.last_event_at),
    last_success_at=coalesce(excluded.last_success_at,pandora_integration_health.last_success_at),
    stale_after=excluded.stale_after,
    details=excluded.details,
    updated_at=now()
  returning * into v_row;
  perform public.pandora_recompute_project(p_organization_id,p_project_key);
  return to_jsonb(v_row);
end;
$$;
grant execute on function public.pandora_set_integration_health(uuid,text,text,text,jsonb,timestamptz,timestamptz,timestamptz) to service_role;
