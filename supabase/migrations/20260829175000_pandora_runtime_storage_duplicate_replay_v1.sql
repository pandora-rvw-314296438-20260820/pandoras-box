-- Forward-only repair: treat Supabase Storage KeyAlreadyExists replay as a duplicate only after exact readback verification.
...
begin
  ...
  if v_existing_artifact_version is not null then
    ...
    insert into public.pandora_artifact_versions(
      ...
      v_version.organization_id,v_version.project_id,v_artifact_id,v_existing_root,1,v_bundle_sha,v_bundle_bytes,'application/json',
...
comment on function private.pandora_finalize_runtime_bundle_20260829(uuid,uuid,uuid,text) is 'Worker D to Worker F exact runtime-bundle finalizer. Source parent lineage is immutable and is written at artifact-version insert time.';
