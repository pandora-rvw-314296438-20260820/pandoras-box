-- Supabase capacity recovery: when Git integration skips declared Edge bulk deploys,\n-- every skipped production function must remain deployable through the exact-source broker.\n-- This forward migration extends the closed allowlist without weakening SHA binding or auth modes.\n-- Pandora release helper: exact source-convergence worker deployment allowlist.
-- Reuses the latest replay-proven canonical helper source and adds one closed allowlist case.
