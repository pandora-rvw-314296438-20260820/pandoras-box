...
as $$
  select
    coalesce((select c.enabled from public.pandora_visible_creation_canary_control c where c.singleton = true), false)
    and exists (
      select 1 from public.pandora_visible_creation_canary_allowlist a
      where a.enabled = true
        and ((a.target_kind = 'user' and a.target_id = p_user_id)
...
comment on table public.pandora_visible_creation_canary_control is 'Task 132 runtime kill switch. enabled=false blocks every new Visible Creation build without a redeploy.';
comment on table public.pandora_visible_creation_canary_allowlist is 'Task 132 bounded user/project allowlist. No client role can read or mutate it.';
comment on function public.pandora_visible_creation_canary_allowed_v1(uuid, uuid) is 'Service-only fail-closed Visible Creation canary decision. Requires global enabled plus an enabled user or project allowlist row.';
