-- Restrict Pandora intelligence routing to phone-local Qwen, Kimi, and the
-- single approved Gemini credential slot (Gemini_api3 / 999 account).
-- No Gemini credential material is returned from Vault.

update public.pandora_runtime_provider_configs
set config_value = case config_key
  when 'enabled' then 'false'
  when 'routing_eligible' then 'false'
  when 'fallback_enabled' then 'false'
  when 'preferred_tasks' then '[]'
  when 'task_eligibility' then '[]'
  else config_value
end,
updated_at = now()
...
begin
  ...
  if nullif(trim(v_key),'') is null then
    raise exception 'approved Gemini provider credential unavailable' using errcode='55000';
  end if;
  ...
  from extensions.http((
    ...
    array[
      ...
      extensions.http_header('user-agent','Pandora-Worker-B-Intelligence/1.2')
  ...
  exception when others then
    v_body := case
      ...
      else jsonb_build_object('raw',left(v_response.content,5000))
    end;
  end;
...
revoke all on function private.pandora_worker_b_gemini_api_20260829(text,jsonb)
  from public, anon, authenticated;
grant execute on function private.pandora_worker_b_gemini_api_20260829(text,jsonb)
  to service_role;
