...
begin
 ...
 values(p_organization_id,p_project_id,'supabase','application_database',v_external,v_schema,v_env,'verified',jsonb_build_object('projectRef','jcyqixttuebxqqfkjonq','schema',v_schema,'isolationMode','isolated_schema'),v_now)
 ...
 values(p_organization_id,p_project_id,p_project_version_id,v_project_resource,'application_database','supabase',v_env,'isolated_schema',v_external,'ap-southeast-1','ready',jsonb_build_object('projectRef','jcyqixttuebxqqfkjonq','schema',v_schema,'databaseRole',v_role,'authorizationRef',p_authorization_ref,'secretStored',false),v_now,v_now)
 ...
 return jsonb_build_object('runtimeResourceId',v_resource,'projectResourceId',v_project_resource,'provider','supabase','projectRef','jcyqixttuebxqqfkjonq','schema',v_schema,'databaseRole',v_role,'isolationMode','isolated_schema','internalTableAccess',false,'secretStored',false,'status','ready');
end;$$;
...
begin
 ...
 select * into v_res from public.pandora_runtime_resources where id=p_runtime_resource_id and resource_type='application_database' and provider='supabase' and isolation_mode='isolated_schema' and status='ready'; if not found then raise exception 'isolated database resource unavailable' using errcode='22023'; end if;
...
begin
 ...
 select * into v_res from public.pandora_runtime_resources where id=v_plan.target_runtime_resource_id and status='ready'; if not found or v_res.isolation_mode<>'isolated_schema' then raise exception 'isolated database unavailable' using errcode='22023'; end if;
