...
begin
  ...
  from vault.decrypted_secrets where name=('pandora_ops_vercel_'||'cron_secret_v1') limit 1;
  if nullif(trim(v_cron_secret),'') is null then
    v_cron_secret:=rtrim(translate(encode(extensions.gen_random_bytes(48),'base64'),'+/','-_'),'=');
    perform vault.create_secret(v_cron_secret,('pandora_ops_vercel_'||'cron_secret_v1'),'Vercel daily Operations cron fallback');
  end if;

  select decrypted_secret into v_hmac_secret
  from vault.decrypted_secrets where name=('pandora_ops_vercel_'||'wake_hmac_v1') limit 1;
  if nullif(trim(v_hmac_secret),'') is null then
    v_hmac_secret:=rtrim(translate(encode(extensions.gen_random_bytes(48),'base64'),'+/','-_'),'=');
    perform vault.create_secret(v_hmac_secret,('pandora_ops_vercel_'||'wake_hmac_v1'),'Supabase to Vercel Operations HMAC wake');
  end if;
...
begin
  if not exists(select 1 from vault.decrypted_secrets where name=('pandora_ops_vercel_'||'wake_hmac_v1') and nullif(trim(decrypted_secret),'') is not null) then
    raise exception 'OPS_WAKE_HMAC_SECRET_UNAVAILABLE' using errcode='55000';
  end if;
