-- Exact-source Edge release allowlist extension for pandora-base44-bridge.
-- Declared in config.toml as enabled=false (capacity skip); release path must remain closed and exact.
...
begin
  ...
  case p_slug
    ...
    when 'pandora-source-convergence-worker' then
      ...
      v_verify_jwt := false;
      v_import_map_path := null;
    when 'pandora-base44-bridge' then
      v_path := 'supabase/functions/pandora-base44-bridge/index.ts';
      v_verify_jwt := false;
      v_import_map_path := 'deno.json';
    else
