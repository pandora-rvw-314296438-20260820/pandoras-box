...
drop policy if exists pandora_integration_secrets_deny_clients on private.integration_secrets; create policy pandora_integration_secrets_deny_clients on private.integration_secrets for all to anon,authenticated using(false) with check(false);
