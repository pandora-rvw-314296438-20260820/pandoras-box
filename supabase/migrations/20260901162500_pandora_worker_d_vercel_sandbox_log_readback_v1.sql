-- Pandora Visible Creation / Chat E — real Worker D command log readback.
-- Extends the existing Vault-backed Vercel Sandbox broker with one read-only
-- command-log endpoint. The provider credential remains server-side in Vault.
...
begin
  ...
  select config_value into strict v_project_id
  from public.pandora_runtime_provider_configs
  ...
  if not (
    ...
    or (upper(p_method)='GET' and v_base ~ '^/v2/sandboxes/sessions/sbx_[A-Za-z0-9]+/cmd/cmd_[A-Za-z0-9]+/logs$')
  ...
  -- Log readback is GET-only and may not carry a request body.
  if v_base ~ '/cmd/cmd_[A-Za-z0-9]+/logs$' and (upper(p_method)<>'GET' or p_body is not null) then
    raise exception 'unsafe Worker D Vercel Sandbox log request' using errcode='22023';
  end if;
  ...
  from extensions.http((
    ...
    array[
      ...
      extensions.http_header('user-agent','Pandora-Worker-D-Sandbox/1.1')
  ...
  exception when others then
    -- Vercel command logs are ND-JSON. Return only a bounded raw transport body;
    -- customer projection applies its own stricter redaction/chunk caps.
    v_body:=case
      when nullif(v_response.content,'') is null then null
      else jsonb_build_object('raw',left(v_response.content,65536))
    end;
  end;
...
revoke all on function private.pandora_worker_d_vercel_sandbox_api_20260829(text,text,jsonb)
  from public,anon,authenticated;
grant execute on function private.pandora_worker_d_vercel_sandbox_api_20260829(text,text,jsonb)
  to service_role;
