-- Pandora static-site full-wire convergence v1.
-- Canonicalizes the deployed governed Worker D static builder and automatically
-- advances exact static-site builds through Worker F preview and independent Worker E verification.
-- Provider credentials remain Vault-backed and are never persisted in source.
...
begin
  ...
  if not found or v_job.job_kind<>'build' or v_job.status<>'queued' or v_job.target_project_version_id is null then
...
revoke all on function public.pandora_worker_d_finalize_static_web_20260830(uuid) from public, anon, authenticated;
grant execute on function public.pandora_worker_d_finalize_static_web_20260830(uuid) to service_role;

create or replace function private.pandora_close_verified_static_build_20260830(
  p_build_job_id uuid,
...
begin
  ...
  if not found or v_job.job_kind<>'build' or v_job.target_project_version_id is null then
    raise exception 'STATIC_BUILD_CLOSURE_JOB_INVALID' using errcode='22023';
  end if;
  select * into v_version from public.pandora_project_versions
   where id=v_job.target_project_version_id and build_job_id=v_job.id
     and organization_id=v_job.organization_id and project_id=v_job.project_id for update;
  if not found then raise exception 'STATIC_BUILD_CLOSURE_VERSION_INVALID' using errcode='22023'; end if;
  select * into v_run from public.pandora_verification_runs
   where id=p_verification_run_id and organization_id=v_job.organization_id and project_id=v_job.project_id
     and project_version_id=v_version.id and build_job_id=v_job.id;
  if not found or upper(v_run.status)<>'PASS' or v_run.required_check_profile<>'static_site'
     or v_run.verifier_identity=coalesce(v_job.worker_identity,'') then
    raise exception 'STATIC_BUILD_CLOSURE_VERIFICATION_INVALID' using errcode='23514';
  end if;

  update public.pandora_build_job_attempts
     set status='completed', finished_at=coalesce(finished_at,v_now)
   where build_job_id=v_job.id and status='running';

  if v_job.status<>'succeeded' then
    update public.pandora_build_jobs
       set status='succeeded', current_stage='preview_ready',
           started_at=coalesce(started_at,(select min(started_at) from public.pandora_build_job_attempts where build_job_id=v_job.id),created_at),
           completed_at=v_now, lease_owner=null, lease_token_sha256=null, lease_expires_at=null,
           heartbeat_at=v_now, error_code=null, public_error_summary=null, updated_at=v_now
     where id=v_job.id and status in ('claimed','running','waiting_verification');
    if not found then raise exception 'STATIC_BUILD_CLOSURE_STATE_INVALID' using errcode='40001'; end if;
  end if;
  return jsonb_build_object('buildJobId',v_job.id,'projectVersionId',v_version.id,'verificationRunId',v_run.id,'state','ready');
end;
$$;
revoke all on function private.pandora_close_verified_static_build_20260830(uuid,uuid) from public,anon,authenticated;
grant execute on function private.pandora_close_verified_static_build_20260830(uuid,uuid) to service_role;
...
begin
  ...
  if v_job.status='queued' then
    perform private.pandora_worker_d_finalize_static_web_20260830(v_job.id);
...
$$;
revoke all on function private.pandora_converge_static_site_build_20260830(uuid) from public,anon,authenticated;
grant execute on function private.pandora_converge_static_site_build_20260830(uuid) to service_role;

create or replace function public.pandora_converge_static_site_build_20260830(p_build_job_id uuid)
returns jsonb language sql security definer set search_path=''
as $$ select private.pandora_converge_static_site_build_20260830($1); $$;
...
returns jsonb language sql security definer set search_path=''
as $$ select private.pandora_converge_pending_static_sites_20260830($1); $$;
...
begin
  ...
  for v_jobid in execute 'select jobid from cron.job where jobname=''pandora-static-site-convergence''' loop
    execute format('select cron.unschedule(%s)',v_jobid);
  end loop;
  execute 'select cron.schedule(''pandora-static-site-convergence'',''* * * * *'',''select private.pandora_converge_pending_static_sites_20260830(5);'')';
end
$cron$;
