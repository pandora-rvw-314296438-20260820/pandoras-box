...
begin
  ...
  with candidates as (
    select
      ...
      s.display_name,
      s.role as role_label,
      coalesce(m.role::text,'staff') as access_role,
      coalesce(m.status::text,'staff_record') as access_status,
      s.active as active,
      'staff_identity'::text as source,
      s.updated_at,
      0 as source_rank
    from plp_runtime.plp_staff_identities s
    left join public.memberships m
      on m.organization_id=prop.organization_id
    ...
    where m.organization_id=prop.organization_id
      and m.status::text='active'
      and lower(coalesce(p.display_name,'')) not like '%staging%'
      and not exists (
        select 1
        from plp_runtime.plp_staff_identities s
        where s.auth_user_id=m.user_id
          and s.active=true
      )
  ),
  ranked as (
  ...
  active_member_count := jsonb_array_length(team_members);
  ...
  return jsonb_build_object(
    'schemaVersion','plp.enterprise.mobile-bootstrap.v4',
    ...
    'teamAccess',jsonb_build_object(
      ...
      'accessModel','organization_membership'
    ),
