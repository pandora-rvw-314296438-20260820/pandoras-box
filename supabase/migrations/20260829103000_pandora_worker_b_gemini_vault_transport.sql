-- Worker B/J: bounded server-side Gemini provider transport.
-- The Gemini API key remains in Supabase Vault and is never returned to callers.
...
create or replace function public.pandora_worker_b_gemini_request_20260829(
  p_model text,
  p_body jsonb
)
returns jsonb
...
set search_path = ''
as $$
  select private.pandora_worker_b_gemini_api_20260829(p_model,p_body);
$$;

revoke all on function public.pandora_worker_b_gemini_request_20260829(text,jsonb) from public, anon, authenticated;
grant execute on function public.pandora_worker_b_gemini_request_20260829(text,jsonb) to service_role;
