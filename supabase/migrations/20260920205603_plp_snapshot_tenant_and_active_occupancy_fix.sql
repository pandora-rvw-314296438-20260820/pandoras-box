-- Forward repair for the already-applied PLP hospitality migrations.
-- Keeps provider state and clean-replay source aligned.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.enterprise_properties'::regclass
      and contype = 'u'
      and pg_get_constraintdef(oid) = 'UNIQUE (id, organization_id)'
  ) then
    alter table public.enterprise_properties
      add constraint enterprise_properties_id_organization_id_key
      unique (id, organization_id);
  end if;
end
$$;

alter table public.enterprise_hospitality_snapshots
  drop constraint if exists enterprise_hospitality_snapshots_property_id_fkey;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.enterprise_hospitality_snapshots'::regclass
      and conname = 'enterprise_hospitality_snapshots_property_org_fkey'
  ) then
    alter table public.enterprise_hospitality_snapshots
      add constraint enterprise_hospitality_snapshots_property_org_fkey
