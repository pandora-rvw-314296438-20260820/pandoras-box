...
begin
  ...
  if coalesce(has_more,false) then
    select p.occurred_at,p.job_id,p.sequence
    into next_before_at,next_before_job_id,next_before_sequence
    from page p
    order by p.occurred_at asc,p.job_id asc,p.sequence asc
    limit 1;
  end if;
