...
begin
  ...
  if v_existing_root is not null then
    update public.pandora_artifact_versions
    set parent_version_id=coalesce(parent_version_id,v_existing_root)
    where id=v_artifact_version_id and parent_version_id is null;
  end if;
...
comment on function private.pandora_finalize_runtime_bundle_20260829(uuid,uuid,uuid,text) is 'Worker D to Worker F exact runtime-bundle finalizer. Authorized source_snapshot root may transition exactly once to runtime_bundle; replay remains byte-exact.';
