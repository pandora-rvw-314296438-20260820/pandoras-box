...
begin
  if auth.role() <> 'service_role'
    ...
    raise exception 'pandora_permanent_account_required' using errcode = '42501';
  end if;
...
comment on function public.pandora_accept_intake(uuid, uuid, text, text, text, text, text, text, text)
is 'Accepts governed Pandora intake for permanent organization members or the service role; anonymous sessions are rejected.';
