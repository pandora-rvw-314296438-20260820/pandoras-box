...
begin
  ...
  insert into public.pandora_intelligence_messages(
    ...
    'pandora_direct_github','gemini-3.1-pro-preview'
  );
...
begin
  ...
  if v_message='Execution handoff persisted; downstream action is not complete.'
    ...
    if coalesce(v_result->>'intent','')='direct_github_code_change'
      ...
      v_repo := coalesce(v_result->'providerReadback'->>'repository','');

      if v_repo='pandora-rvw-314296438-20260820/pandoras-box' then
        v_summary := 'Pandora source change opened in GitHub and provider readback verified.';
      elsif v_repo='pandora-rvw-314296438-20260820/plp' then
        v_summary := 'PLP code change committed directly to GitHub and provider readback verified.';
      else
        v_summary := 'Code change completed at the GitHub provider boundary and readback verified.';
      end if;

      v_event := jsonb_set(v_event,'{message}',to_jsonb(v_summary),false);
      if jsonb_typeof(v_event->'outcome')='object' then
        v_event := jsonb_set(v_event,'{outcome,summary}',to_jsonb(v_summary),false);
      end if;
    end if;
  end if;
