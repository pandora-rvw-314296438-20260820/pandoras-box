-- Owner-authorized reversible production hold for Operations Room.
-- Keeps revision fencing, owner/admin admission and immutable control events.
...
begin
 ...
 else raise exception 'OPS_CONTROL_NOT_REGISTERED';
 end if;

 update private.pandora_ops_workspaces
 set paused=w.paused,
     no_production=w.no_production,
     revision=revision+1,
 ...
 perform private.pandora_ops_event_v1(
   ...
   'control:'||w.revision::text,p_task_key,'owner_'||p_action
 );
