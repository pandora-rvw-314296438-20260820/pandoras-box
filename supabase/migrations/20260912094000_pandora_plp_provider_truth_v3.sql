...
-- CHAT-FINISH-002
-- Provider truth supersedes the earlier PLP repository assertion. On 2026-09-12,
-- both the exact GitHub repository identity (including immutable repo id 1358856339)
...
set binding_state = 'degraded',
    configuration = coalesce(configuration, '{}'::jsonb) || jsonb_build_object(
      ...
      'provider_readback_observed_at', '2026-09-12T09:35:00Z',
      'truth_correction_task', 'CHAT-FINISH-002',
      'truth_correction_reason', 'github_repository_not_found_by_full_name_or_repository_id'
    ),
    verified_at = null,
    updated_at = now()
where organization_id = '2270b266-59da-4c39-bfd9-9f8d08352af0'::uuid
  and project_id = '73b1afe9-91b5-4bf5-864c-22c071c4471a'::uuid
  and provider = 'github'
...
set binding_state = 'degraded',
    configuration = coalesce(configuration, '{}'::jsonb) || jsonb_build_object(
      ...
      'provider_readback_observed_at', '2026-09-12T09:35:00Z',
      'truth_correction_task', 'CHAT-FINISH-002',
      'truth_correction_reason', 'vercel_project_not_found_under_current_team'
    ),
    verified_at = null,
    updated_at = now()
where organization_id = '2270b266-59da-4c39-bfd9-9f8d08352af0'::uuid
  ...
  and external_id = 'prj_4W4GcwFJ3BPA4TnsEfHkmAm7HYeV';
...
begin
  ...
  if v_message ~ '\mplp\M|plp[- ]boracay|pueblo la perla' then
    ...
    if not found then
      return jsonb_build_object(
        'state','degraded','resolved',false,'resolution','canonical_repository_alias',
        'repository',null,'projectId',null,'projectKey','plp-boracay',
        'repositoryStatus','degraded','blockedReason','project_binding_missing','projectRequired',false
      );
    end if;
    ...
    where r.organization_id=p_organization_id
      ...
      and r.resource_type='repository'
    order by r.updated_at desc limit 1;

    if not found
       or v_resource.binding_state <> 'verified'
       or nullif(trim(v_resource.external_id),'') is null then
      return jsonb_build_object(
        'state','degraded','resolved',false,'resolution','canonical_repository_alias',
        'repository',null,'projectId',v_project.id,'projectKey',v_project.project_key,
        'projectName',v_project.name,'repositoryStatus','degraded',
        'blockedReason','provider_readback_required','projectRequired',false
      );
    end if;

    return jsonb_build_object(
      'state','resolved','resolved',true,'resolution','canonical_repository_alias',
      'repository',v_resource.external_id,
      'projectId',v_project.id,'projectKey',v_project.project_key,
      'projectName',v_project.name,'repositoryStatus','provider_verified',
      'repositoryId',v_resource.configuration->'repository_id','projectRequired',false
    );
  end if;
...
begin
  ...
  if v_actionable and coalesce(v_target->>'state','')='degraded' then
    ...
    v_reply := format(
      'I resolved this to %s, but its current GitHub repository binding failed provider readback. I did not guess a replacement repository or execute a mutation. Restore or rebind the verified repository and I can continue in this chat.',
      coalesce(v_target->>'projectName',v_target->>'projectKey','the requested project')
    );
    ...
    values(v_thread_id,p_organization_id,v_target_project_id,'assistant',v_reply,
      jsonb_build_object(
        ...
        'capabilityResult',jsonb_build_object(
          'ok',false,'state','blocked','reason','repository_binding_degraded',
          'provider','github','authorityGranted',false
        ),
        'handoff',null
    ...
    return jsonb_build_object(
      ...
      'intent','repository_action','confidence',1,'needsClarification',false,'clarifyingQuestion',null,
      'repositoryTarget',v_target,'projectRequired',false,'handoff',null,
      'capabilityResult',jsonb_build_object(
        'ok',false,'state','blocked','reason','repository_binding_degraded',
        'provider','github','authorityGranted',false
      )
...
is 'Universal Chat repository targeting v3: PLP provider truth is dynamic and degraded bindings are handled inline without mutation or fallback guessing.';
