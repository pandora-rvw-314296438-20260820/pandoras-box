...
async function readbackWorkerDSource(
  admin: ReturnType<typeof adminClient>,
  buildJobId: string,
) {
  ...
  if (av.content_sha256 !== v.source_sha256 || av.storage_provider !== "supabase_storage" ||
      ...
      Number(av.byte_size) <= 0 || Number(av.byte_size) > MAX_READBACK_SOURCE_BYTES ||
  ...
  try {
    ...
    throw new Error("SOURCE_READBACK_INVALID");
  }
  ...
  return {
    ...
    sha256: av.content_sha256,
    byteSize: Number(av.byte_size),
    sourceText,
  };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (exactKeys(body, ["buildJobId"]) && UUID.test(text(body.buildJobId))) {
      try {
        return response(await readbackWorkerDSource(admin, text(body.buildJobId)), 200);
      } catch (error) {
        const code = error instanceof Error ? error.message : "SOURCE_READBACK_FAILED";
        const status = code === "BUILD_JOB_NOT_FOUND" ? 404 :
          code === "BUILD_LEASE_INVALID" ? 409 :
          code === "BUILD_VERSION_INVALID" || code === "SOURCE_ARTIFACT_INVALID" ? 409 : 503;
