...
async function streamGeminiSource(admin: ReturnType<typeof adminClient>, providerRequest: JsonRecord, state: StreamAssembler) {
  ...
  return {
    ...
    meta: {
      ...
      providerLatencyMs: measuredFirstByteAt === null
        ? null
        : Math.max(0, Math.round(measuredFirstByteAt - responseHeadersAt)),
      timeToFirstTokenMs: measuredFirstByteAt === null
        ? null
        : Math.max(0, Math.round(measuredFirstByteAt - providerStartedAt)),
      streamCompletionLatencyMs: measuredFirstByteAt === null
        ? null
        : Math.max(0, Math.round(providerCompletedAt - measuredFirstByteAt)),
