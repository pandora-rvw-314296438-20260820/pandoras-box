...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(!sourceAllowed){
      ...
      return json({kind:"pandora.mobile-preview-bundle.v1",projectId,versionId,artifactDigest,previewDeploymentId,sourceSha256:sourceSha,sourceCommitSha,totalBytes:wrapperBytes.byteLength,sourceIncluded:false,sourceEntitled:false,hostedPreview:{deploymentId:deployment.id,url:hostedUrl,sourceSha256:sourceSha,sourceCommitSha,artifactDigest},files:[{file:"index.html",mimeType:"text/html",dataBase64:base64Bytes(wrapperBytes),byteSize:wrapperBytes.byteLength,sha256:wrapperDigest,artifactDigest,previewProjectId:projectId,previewVersionId:versionId,previewDeploymentId,sourceSha256:sourceSha,sourceCommitSha}]},200,requestId);
    }
