...
class PandoraDeviceCommunicationCommand {
  const PandoraDeviceCommunicationCommand(
      {required this.kind, required this.recipient, this.message});
  ...
  static PandoraDeviceCommunicationCommand? tryParse(String raw) {
    ...
    final call = RegExp(r'^(?:please\s+)?(?:call|dial)\s+(.+?)\s*$',
            caseSensitive: false)
        .firstMatch(input);
    if (call != null) {
      ...
      return PandoraDeviceCommunicationCommand(
          kind: PandoraCommunicationKind.call, recipient: recipient);
    }
    final text =
        RegExp(r'^(?:please\s+)?(?:text|sms)\s+(.+?)\s*$', caseSensitive: false)
            .firstMatch(input);
    final reply =
        RegExp(r'^(?:please\s+)?reply\s+to\s+(.+?)\s*$', caseSensitive: false)
            .firstMatch(input);
    ...
    final saying = RegExp(r'^(.+?)\s+saying\s+(.+)$', caseSensitive: false)
        .firstMatch(rest);
    if (saying != null) {
  ...
  static String _cleanRecipientLabel(String value) {
    ...
    result =
        result.replaceFirst(RegExp(r'\s+now[.!?]*$', caseSensitive: false), '');
