      ...
      builder: (context, _) {
        ...
        if (approvals.isEmpty) {
          return const EmptyContent(
            ...
            message: 'Pandora will bring work here only when your judgment is required.',
        ...
        return Column(
          ...
          children: [
            ...
            const OwnerSectionHeading(
              ...
              subtitle: 'See what changes and whether it can be undone.',
            ),
        ...
        child: Column(
          ...
          children: [
            ...
            OwnerSignal(
              label: 'Rollback',
              value:
                  approval.rollback ?? 'No verified rollback path is recorded.',
              icon: Icons.restore_rounded,
              tone: approval.rollback != null
                  ? PandoraStatusTone.verified
                  : PandoraStatusTone.critical,
            ),
            const SizedBox(height: PandoraSpacing.lg),
            ...
            const SizedBox(height: PandoraSpacing.lg),
            const StatusBadge(
              label: 'Environment proof: not verified',
              tone: PandoraStatusTone.attention,
            ),
            const SizedBox(height: PandoraSpacing.xs),
            const StatusBadge(
              label: 'Cost proof: not verified',
              tone: PandoraStatusTone.attention,
            ),
            const SizedBox(height: PandoraSpacing.xs),
            StatusBadge(
              label: approval.expiresAt == null
                  ? 'Expiry proof: not verified'
                  : 'Expiry recorded',
              tone: approval.expiresAt == null
                  ? PandoraStatusTone.attention
                  : PandoraStatusTone.neutral,
            ),
          ],
...
String _sanitizeApprovalText(String input) {
  ...
  value = value.replaceAll(
    RegExp(r'\b(?:gh[pousr]_[A-Za-z0-9_]{20,}|sb_secret_[A-Za-z0-9_]{20,})\b'),
    '[redacted]',
  );
  return value;
}
