...
class _SimpleBusinessScreenState extends State<SimpleBusinessScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _projects = snapshot.data.where(isOwnerVisibleProject).toList(growable: false);
        _loading = false;
      });
  ...
  Widget build(BuildContext context) {
    final live = _projects.where(
      (project) =>
          project.evidenceState(EvidenceStage.productionVerified) ==
              EvidenceClaimState.verified &&
          project.freshness.isFresh,
    ).length;
    final needsYou = _projects.where(
      (project) => project.blocker?.trim().isNotEmpty == true,
    ).length;
    final working = (_projects.length - live - needsYou).clamp(0, _projects.length);
