...
class _SimpleHomeScreenState extends State<SimpleHomeScreen> {
  ...
  String _state(ProjectSummary p) {
    ...
    final live = p.evidenceState(EvidenceStage.productionVerified) == EvidenceClaimState.verified && p.freshness.isFresh;
    if (live) return 'Live';
      ...
      child: PandoraV2Page(
        child: Column(
          ...
          children: [
            ...
            const Text('Good afternoon, Mark', style: TextStyle(color: PandoraV2Colors.muted, fontSize: 16)),
            const SizedBox(height: 8),
            const Text(
              'What do you want\nto make happen?',
              style: TextStyle(color: PandoraV2Colors.ink, fontSize: 36, fontWeight: FontWeight.w700, letterSpacing: -1.35, height: 1.02),
            ),
            ...
            PandoraV2IntentSurface(
              ...
              onAttachment: () async {
                ...
                if (file != null && mounted) _intent.text = '${_intent.text.trim()}\n${file.promptBlock}'.trim();
              },
            ),
            const SizedBox(height: 42),
            Row(children: [
              const Text('Your work', style: TextStyle(color: PandoraV2Colors.ink, fontSize: 20, fontWeight: FontWeight.w700)),
              const Spacer(),
              if ((summary?.topProjects.length ?? 0) >= 3 && widget.onOpenSystems != null)
                TextButton(onPressed: widget.onOpenSystems, style: TextButton.styleFrom(foregroundColor: PandoraV2Colors.ink), child: const Text('All work')),
            ]),
            if (_loading) ...[
              ...
              PandoraV2InlineMessage(title: 'Your work is still safe', message: _error ?? 'Pandora could not load it yet.', actionLabel: 'Try again', onAction: _load)
            else if (summary.topProjects.isEmpty)
              PandoraV2InlineMessage(title: 'Nothing here yet', message: 'Describe what you want above and Pandora will create the first working version.')
            else
              for (final project in summary.topProjects)
                PandoraV2ObjectWindow(
                  ...
                  trailing: _openingId == project.id ? const SizedBox.square(dimension: 20, child: CircularProgressIndicator(strokeWidth: 2, color: PandoraV2Colors.ink)) : null,
                ),
            ...
            if (summary != null && summary.countersVerified && summary.approvalCount > 0) ...[
              const SizedBox(height: 38),
              const Text('Needs you', style: TextStyle(color: PandoraV2Colors.ink, fontSize: 20, fontWeight: FontWeight.w700)),
              PandoraV2InlineMessage(
                title: summary.approvalCount == 1 ? 'One decision is waiting for you' : '${summary.approvalCount} decisions are waiting for you',
