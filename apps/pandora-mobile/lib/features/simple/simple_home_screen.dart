...
class _WorkingRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final testing = normalized.contains('test') || normalized.contains('review');
    final healthy = normalized.contains('complete') || normalized.contains('live');
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final count = width >= 690 ? 4 : width >= 420 ? 2 : 1;
        ...
        return Wrap(
          ...
          children: [
            for (final metric in metrics)
              SizedBox(width: itemWidth, child: _PulseCard(metric: metric)),
          ],
        );
      },
    );
