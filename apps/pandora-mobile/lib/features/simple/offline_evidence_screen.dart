...
class _OfflineEvidenceScreenState extends State<OfflineEvidenceScreen> {
  ...
  Widget build(BuildContext context) => PandoraSimplePage(
    header: PandoraOwnerHeader(
      ...
      subtitle: 'Read-only proof you can inspect without authorizing a change.',
      showBack: true,
        ...
        ? PandoraSimpleCard(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  _error ?? 'Open Pandora while connected first so bounded read-only evidence can be retained.',
                ),
    ...
    children: [
      ...
      PandoraSimpleCard(
        ...
        child: const Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Icon(Icons.lock_clock_outlined, color: PandoraSimpleColors.amber),
            SizedBox(width: 10),
            Expanded(
              child: Text(
                'Offline evidence is informational only. Refresh live provider state before approving, releasing, or changing a system.',
                style: TextStyle(color: PandoraSimpleColors.ink, height: 1.35),
              ),
            ),
          ],
        ),
