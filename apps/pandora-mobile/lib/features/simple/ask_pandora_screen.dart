...
class AskPandoraScreenState extends State<AskPandoraScreen> {
  ...
  Future<void> _watchActivity(PandoraIntelligenceExecution execution) async {
    ...
    _activitySubscription = execution.events.listen(
      ...
      onError: (Object _, StackTrace __) {
        if (!mounted || _activeActivityJobId != execution.jobId) return;
        setState(() {
          _error = 'Live Activity disconnected. Pandora kept the persisted activity history for recovery.';
        });
      },
    );
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final turnRequestId = _submissionKey ??= _keys.create('pandora-chat-turn');
      ...
      setState(() {
        ...
        _liveActivityMessage = null;
        if (turn.handoff == null) _submissionKey = null;
      });
...
class _PandoraThinkingBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(
              message?.trim().isNotEmpty == true ? message!.trim() : 'Thinking…',
              style: const TextStyle(
                color: PandoraSimpleColors.muted,
                fontSize: 14,
              ),
