...
class _ExactSourceVerificationCardState
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      setState(() {
        ...
        _error =
            'Pandora could not confirm the outcome. This app will not '
  ...
  Future<void> _readExecution(String planId) async {
    ...
    _polling = true;
    try {
      final execution = await PandoraDependencies.of(context).repository
      ...
      if (mounted) {
        _ensurePolling(planId);
        setState(() {
          _readError = 'The exact plan is recorded, but its latest proof could not be read.';
        });
      }
    } finally {
      _polling = false;
