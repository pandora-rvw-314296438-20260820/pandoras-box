#!/usr/bin/env python3
"""Configure the disposable Flutter Android project as dedicated PLP Enterprise."""
...
def configure(gradle: Path, manifest: Path, kotlin_root: Path) -> int:
    ...
    for path in kotlin_files:
        ...
        if _GENERIC_PACKAGE not in text:
            continue
        updated = text.replace(_GENERIC_PACKAGE, _PLP_PACKAGE)
    ...
    if _GENERIC_PACKAGE in gradle_verified:
        ...
        return 1
    if f'namespace = "{_PLP_PACKAGE}"' not in gradle_verified:
        print("PLP Android namespace verification failed.", file=sys.stderr)
        return 1
    if f'applicationId = "{_PLP_PACKAGE}"' not in gradle_verified:
        print("PLP Android applicationId verification failed.", file=sys.stderr)
        return 1
    if _PLP_LABEL not in manifest_verified or _GENERIC_LABEL in manifest_verified:
        print("PLP Android application label verification failed.", file=sys.stderr)
        return 1
...
def main() -> int:
    if len(sys.argv) != 4:
        print(
            "usage: configure_plp_android.py "
