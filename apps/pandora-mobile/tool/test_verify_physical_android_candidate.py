...
class PhysicalAndroidCandidateVerifierTest(unittest.TestCase):
    def test_manifest_binding_requires_external_gates_to_remain_false(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            api.write_bytes(b"pandora")
            digest = hashlib.sha256(b"pandora").hexdigest()
            manifest = {
                "source_sha": "a" * 40,
                "apk_sha256": digest,
                "android_package": verifier.EXPECTED_PACKAGE,
                "app_version": "0.4.0-rc.2+8",
            ...
            verifier.require_manifest_binding(
                manifest,
                source_sha="a" * 40,
                apk_sha256=digest,
                package_name=verifier.EXPECTED_PACKAGE,
                app_version="0.4.0-rc.2+8",
            )
    ...
    def test_badging_parser_binds_package_and_version(self):
        package, version_name, version_code = verifier.parse_badging(
            "package: name='com.banataosystems.pandora_mobile' "
            "versionCode='8' versionName='0.4.0-rc.2'"
        )
        self.assertEqual(package, verifier.EXPECTED_PACKAGE)
        self.assertEqual(version_name, "0.4.0-rc.2")
        self.assertEqual(version_code, "8")
