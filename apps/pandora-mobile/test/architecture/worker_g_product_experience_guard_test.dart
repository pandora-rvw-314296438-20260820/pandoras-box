...
void main() {
  ...
  test('object-first build experience uses durable runtime truth', () {
    ...
    expect(source, contains('simplePublishEligibleVersion('));
    expect(source, contains('projectionCanPublish: _projection?.canPublish == true'));
    expect(source, contains('candidateIsVisible: _versionRoles?.candidateIsVisible == true'));
    expect(source, contains('runtimeCandidateVersionId: _snapshot?.candidate?.versionId'));
    expect(source, contains('verificationVersionId: _snapshot?.verification?.versionId'));
    expect(source, contains('runtimePublishEligible: _snapshot?.verification?.publishEligible == true'));
    expect(source, contains('_publishVersionId != publishVersionId'));
