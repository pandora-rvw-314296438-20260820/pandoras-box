...
void main() {
  test(
    'exact diff is bound to verified baseline and candidate source bundles',
    () {
      ...
      expect(
        source,
        contains('ProjectExactSourceDiff.fromExactPreviewBundles('),
      );
      expect(source, contains('projection.currentVerified'));
      expect(
        source,
        contains('projection.currentVersionId != baselineVersionId'),
      );
      expect(
        source,
        contains('projection.candidateVersionId != candidateVersionId'),
      );
