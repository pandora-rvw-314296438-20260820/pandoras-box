...
void main() {
  test('Create Project follows the owner-first three-step journey', () {
    ...
    final projects = File(
      'lib/features/simple/projects_screen.dart',
    ).readAsStringSync();
    final legacy = File(
      'lib/features/simple/project_journey_flow.dart',
    ).readAsStringSync();
    ...
    expect(
      projects,
      contains('const CreateProjectExperienceScreen()'),
    );
  ...
  test('understanding is bound to the exact submitted intent', () {
    final source = File(
      'lib/core/data/project_experience_api.dart',
    ).readAsStringSync();
