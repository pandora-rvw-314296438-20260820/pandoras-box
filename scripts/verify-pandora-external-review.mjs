...
const REVIEW_MARKER = /<!--\s*pandora-external-review-issue\s*:\s*([A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+)#(\d+)\s*-->/gi;
const REPORT_LINK = /https:\/\/github\.com\/([A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+)\/pull\/(\d+)/g;
const REPORT_TITLE = /^pandora-report-head: ([0-9a-f]{40})$/;
...
function assertExactReviewIssue(issue, target, targetTree, repository) {
  ...
  assert(
    exactStructuredValue(issue.body, "pandora-target", /[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+#\d+/)
      === `${repository}#${target.number}`,
    "external-review issue target is not exact",
  );
  assert(
    exactStructuredValue(issue.body, "pandora-target-base", /[0-9a-f]{40}/) === target.base.sha,
    "external-review issue base is not exact",
  );
  assert(
    exactStructuredValue(issue.body, "pandora-target-head", /[0-9a-f]{40}/) === target.head.sha,
    "external-review issue head is not exact",
  );
  assert(
    exactStructuredValue(issue.body, "pandora-target-tree", /[0-9a-f]{40}/) === targetTree,
...
function assertReportPull(report, reference, target, repository) {
  ...
  assert(titleMatch?.[0] === title && titleMatch[1] === report.head.sha, "initial report PR title must be exactly pandora-report-head: <exact report head SHA>");
}
...
export async function main() {
  ...
  assert(
    exactStructuredValue(content, "pandora-target", /[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+#\d+/)
      === `${repository}#${target.number}`,
    "report does not name the exact target PR",
  );
  assert(
    exactStructuredValue(content, "pandora-target-base", /[0-9a-f]{40}/) === target.base.sha,
    "report does not name the exact target base",
  );
  assert(
    exactStructuredValue(content, "pandora-target-head", /[0-9a-f]{40}/) === target.head.sha,
    "report does not name the exact target head",
  );
  assert(
    exactStructuredValue(content, "pandora-target-tree", /[0-9a-f]{40}/) === targetTree,
  ...
  const verdicts = nonblank.filter((line) => /^\s*pandora-verdict\s*:/i.test(line));
  assert(
    verdicts.length === 1 && /^pandora-verdict: (pass|changes-requested|blocked)$/.test(verdicts[0]),
    "report must contain exactly one exact pandora verdict",
  );
  assert(verdicts[0] === "pandora-verdict: pass", `review verdict is ${verdicts[0]?.slice("pandora-verdict: ".length) || "invalid"}`);
...
if (import.meta.url === directPath) {
  main().catch((error) => {
    ...
    summary(["### Pandora external review", "", "Status: FAIL", "", `- ${message}`]);
