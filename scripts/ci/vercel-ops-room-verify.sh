...
OUT=vercel-ops-room-proof

test "$(git rev-parse HEAD:apps)" = "$EXPECTED_APPS_TREE"
test "$(git rev-parse HEAD:apps/pandora-mobile/lib/features/operations/operations_room_screen.dart)" = "$SCREEN_BLOB"
test "$(git rev-parse HEAD:apps/pandora-mobile/test/features/operations/operations_room_test.dart)" = "$TEST_BLOB"
test "$(git rev-parse HEAD:apps/pandora-mobile/pubspec.lock)" = "$LOCK_BLOB"
...
cd ../..
{
  ...
  echo "verification_lane=vercel-isolated-git-manual"
} > "$OUT/manifest.txt"

cat > "$OUT/index.html" <<EOF
<!doctype html><meta charset="utf-8">
<title>Pandora Operations Room exact-source verification</title>
<h1>PASS - Pandora Operations Room exact-source verification</h1>
