...
Pandora may launch a protected app, use supported deep links/APIs, explain required user steps, and resume after supported user re-authentication. A protected deep link is allowed only when its target is allowlisted, it is independently verified side-effect-free, and it carries no sensitive payload. Safe handoff never authorizes private screen scraping or financial side effects.
...
A model suggestion, Memory pattern, prediction, prior success, or provider fallback never grants protected-app authority. Authority, user-presence, provider-control, allowlist and verification facts supplied to this policy must come from trusted runtime evaluation; model/client/device payloads may not self-assert those facts.

## Authority and financial actions

- **F0 information_only** — non-transactional public information or guidance.
- **F1 protected_app_handoff** — launch or verified side-effect-free supported deep link; no financial side effect.
...
F2/F3 require current explicit user authority or an active explicit standing policy whose scope **and bounds are both verified**, plus protected-app user presence where required. For standing financial authority, the applicable amount/limit, recipient/payee, account, provider and validity bounds must match explicitly; missing bound evidence fails closed. Account-security changes, authenticator migration and password export require current explicit authority and cannot rely on standing authority. Authority never overrides F4.

Sensitive protected reads require valid authority, user presence, and verified provider/OS protections. Consequential protected mutations—including transfers, purchases, payee/account changes, authenticator migration, password export, and generic supported protected API actions—also require an explicit post-action verification plan. If a required control is missing, Pandora fails closed.
...
Known violations are **blocked**. Unknown or untrusted integrity facts are **needs_physical_verification**. Only trusted runtime evidence plus explicitly verified facts and physical evidence may reach **meets_policy_baseline**.
...
Factory reset, destructive reprovisioning, bootloader changes, or equivalent destructive work must not begin unless the recovery/authority facts come from trusted runtime policy context and every gate is verified: authenticator recovery, password-manager recovery, protected-app recovery, backup/export, SIM/eSIM recovery, a known-good rollback path, and explicit user authorization for the destructive action.
...
Secret-like fields such as credentials, OTPs, tokens, cookies, authorization headers, API keys, private keys and passwords are redacted. Redaction also detects common secret shapes inside otherwise innocently named string fields. Email/phone-shaped personal values are redacted even under neutral field names, and personal payload fields are redacted by default. Unsafe object keys and unsafe/secret-shaped evidence references are stored only as SHA-256-derived identifiers. Audit traversal is depth/cycle bounded. Raw idempotency keys are never stored.
