...
Vercel reports the current repaired production deployment as a valid rollback candidate. Because it is the only production history entry on the disposable proof project, a preview cannot be used as a rollback target: the provider correctly rejects a deployment that has never served production traffic. The next safe sequence is therefore to wait for one rolling-window slot, create a second production deployment from the exact repaired source/artifact, verify it, roll back to `dpl_AYi7Q9KGMYJ8kRRa8qqPvcW569Ju`, reverify the restored source/artifact/runtime, then restore the second repaired production and reverify again.

The earliest currently observed rolling slot is approximately **2026-08-30 04:46:07 Manila time**. Until that exact rollback/restore sequence passes, publish eligibility remains false and PR #107 must not merge. Worker A records this narrow release blocker in run `bb249a11-20f8-4a1d-9bd3-2c8c82d37ec7`.
