...
`code_chunk`, `file_started`, and `file_completed` are ephemeral and expire after 20 minutes.
...
10. If `historyGapDueToRetention` is true, do not fabricate expired source. Render the durable summary/current stage and continue with surviving real events.
