...
test('active chat dispatcher retires the Pandora fallback', () => {
  const migration = read('supabase', 'migrations', '20260914050000_pandora_pandora_retirement_v1.sql');
  const body = migration.split('as $body$')[1]?.split('$body$;')[0] ?? '';
  assert.ok(body.length > 1000);
  assert.doesNotMatch(body, /pandora_chat_universal_dispatch_v8/i);
  assert.doesNotMatch(body, /pandora_accept_intake/i);
  assert.doesNotMatch(body, /source['\s,:]+pandora_intake/i);
  assert.match(body, /pandora_native_intelligence/);
  assert.match(body, /project_workspace_change/);
});

test('production execution ledger no longer auto-enrolls work into Pandora', () => {
  ...
  assert.doesNotMatch(ledger, /new mandatory_intake_js_1\.PandoraExecutionIntakeProvider/);
  assert.doesNotMatch(ledger, /Mandatory Pandora intake is required/);
});
