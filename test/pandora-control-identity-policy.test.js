...
test("pandora-control accepts only the recovered production Vercel identity", async () => {
  const { assertProductionVercelClaims } = await import(
    "../supabase/functions/pandora-control/identity-policy.mjs"
  );
...
test("pandora-control rejects non-production or mismatched Vercel claims", async () => {
  const { assertProductionVercelClaims } = await import(
    "../supabase/functions/pandora-control/identity-policy.mjs"
  );
