...
const {
  canApprovePandoraPlan,
  pandoraApproverAttribution,
...
const ALL_SCOPES = ['openid', 'email', 'profile', 'offline_access', 'pandora:read', 'pandora:plan', 'pandora:approve', 'pandora:execute'];
...
for (const role of ['owner', 'admin']) {
  test(`authenticated ${role} AAL1 can approve an eligible pending plan`, async () => {
    ...
    const request = requestFor('pandora_approve_plan', { planId: PLAN_ID });
...
test('invalid plan identity is rejected before the durable ledger', async () => {
  const { handler, calls } = harness('owner');
  const response = await invoke(handler, requestFor('pandora_approve_plan', { planId: 'not-a-uuid' }));
...
test('project-scoped OAuth grants require the exact action scope and cannot fall back to legacy compatibility', async () => {
  for (const [name, scopes, required] of [
    ['pandora_approve_plan', [...IDENTITY_SCOPES, 'pandora:read'], 'pandora:approve'],
    ['pandora_execute_plan', [...IDENTITY_SCOPES, 'pandora:approve'], 'pandora:execute'],
    ['pandora_approve_plan', [...IDENTITY_SCOPES, 'unrecognized:scope'], 'pandora:approve'],
  ]) {
    const { ledger } = harness('owner');
    const handler = createPandoraMcpHandler({
      organizationId: ORGANIZATION_ID,
    ...
    authenticator: {
      async authenticate() {
        return {
          ...
          accessToken: ACCESS_TOKEN,
          aal: 'aal1',
          scopes: [...IDENTITY_SCOPES, 'pandora:*'],
          scopeClaimsPresent: true,
        };
      },
    },
...
test('an approved plan is claimed and executed once and replay is rejected', async () => {
  ...
  claimed = false;
  const executeHandler = createPandoraMcpHandler(dependencies);
  const first = await invoke(executeHandler, requestFor('pandora_execute_plan', { planId: PLAN_ID }));
  const replay = await invoke(executeHandler, requestFor('pandora_execute_plan', { planId: PLAN_ID }));
...
test('Pandora MCP delete execution passes a ledger-backed reservation callback into provider configuration', async () => {
  ...
  const response = await invoke(handler, requestFor('pandora_execute_plan', { planId: PLAN_ID }));
  assert.equal(response.statusCode, 200);
...
test('approval authorization depends on owner/admin role, not AAL', () => {
  ...
  assert.equal(canApprovePandoraPlan(ownerAal1), true);
  assert.equal(canApprovePandoraPlan(adminWithoutAal), true);
  assert.equal(pandoraApproverAttribution(ownerAal1), `supabase:${USER_ID}`);
  assert.doesNotMatch(pandoraApproverAttribution(ownerAal1), /aal/i);
});
