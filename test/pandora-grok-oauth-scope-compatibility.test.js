...
function handlerWithScopes(scopes) {
  return createPandoraMcpHandler({
...
test("protected-resource metadata advertises Pandora action scopes", async () => {
...
test("Grok standard identity grant with phone remains bounded-legacy compatible", async () => {
  ...
  const response = await invoke(handler, {
    ...
    body: {
      ...
      id: 1,
      method: "tools/call",
      params: { name: "pandora_tool_catalog", arguments: {} },
    },
  });
  assert.equal(response.statusCode, 200);
...
test("unrelated non-Pandora scopes remain fail closed", async () => {
