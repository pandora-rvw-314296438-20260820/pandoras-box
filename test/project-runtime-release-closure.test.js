...
test("Supabase management credential stays behind the security-definer broker", () => {
  ...
  const brokerStart = sql.indexOf("create or replace function private.pandora_release_deploy_edge_from_github_20260829");
  const brokerEnd = sql.indexOf("revoke all on function private.pandora_release_deploy_edge_from_github_20260829", brokerStart);
  const broker = sql.slice(brokerStart, brokerEnd);
  assert.ok(brokerStart >= 0 && brokerEnd > brokerStart);
  assert.match(broker, /v_token:=null;\s*return jsonb_build_object\(/i);
  const returned = broker.slice(broker.lastIndexOf("return jsonb_build_object("));
  assert.doesNotMatch(returned, /decrypted_secret|v_token|authorization|bearer|_pat\b/i);
});
