...
test('M1-004 creates neutral durable activity transport with replay and realtime', () => {
  ...
  assert.match(migration, /pandora_activity_admit_projection_v1/);
...
test('event admission is ordered, terminal-safe, and stores canonical public projections', () => {
  assert.match(migration, /v_sequence <> v_job\.last_sequence \+ 1/);
  assert.match(migration, /v_job\.terminal_state is not null/);
  assert.match(migration, /p_projection->>'projectionVersion'/);
  assert.match(migration, /p_projection->>'jobId'/);
...
test('intelligence runtime emits real admitted, model, checking, and verified result events', () => {
  ...
  assert.match(edgeActivity, /pandora_activity_admit_projection_v1/);
});
...
test('transport never grants client write access to canonical activity events', () => {
  ...
  assert.doesNotMatch(mobileApi, /pandora_activity_admit_projection_v1/);
});
