...
const migration = readFileSync(
  join(root, 'supabase/migrations/20260901094550_pandora_build_watchdog_v1.sql'),
  'utf8',
);
...
test('hard deadline adjudication happens before retryable lease requeue', () => {
  const deadline = migration.indexOf('-- deadline_at is the server-owned hard stop');
  const requeue = migration.indexOf('-- Existing lease authority remains canonical for retryable expirations.');
  assert.ok(deadline >= 0);
  assert.ok(requeue > deadline);
});
