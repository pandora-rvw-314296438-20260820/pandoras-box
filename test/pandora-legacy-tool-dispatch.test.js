...
test("tools/list preserves cached provider reads and durable plan aliases", async () => {
  ...
  assert.ok(tools.has("pandora_plan_github_write-repository-api"));
  assert.ok(tools.has("pandora_plan_supabase_write-child-database-query"));
  assert.equal(
    tools.get("pandora_plan_supabase_write-child-database-query").inputSchema.additionalProperties,
    false,
  );
  assert.ok(tools.has("pandora_plan_supabase_read-child-database-query"));
  assert.equal(
    tools.get("pandora_plan_supabase_read-child-database-query").inputSchema.additionalProperties,
    false,
  );
  assert.ok(tools.has("pandora_plan_supabase_delete-child-branch"));
  assert.equal(
    tools.get("pandora_plan_supabase_delete-child-branch").inputSchema.additionalProperties,
    false,
  );
  ...
  assert.ok(tools.has("pandora_approve_plan"));
  assert.ok(tools.has("pandora_list_audit"));
  assert.ok(tools.has("pandora_verify_audit"));
  assert.equal(tools.has("github.write-repository-api"), false);
  assert.equal(tools.has("pandora_plan_flutterflow_inspect-readiness"), false);
});
...
test("cached plan aliases bind the underlying mutation without executing it", async () => {
  ...
  const response = await invoke(handler, request("tools/call", {
    name: "pandora_plan_github_write-repository-api",
    arguments: args,
  }));
  assert.equal(response.statusCode, 200);
...
test("cached direct mutation names cannot bypass durable planning", async () => {
  ...
  assert.match(response.body.error.message, /durable Pandora plan is required/i);
