...
test('active universal router uses Pandora-native capability dispatch', () => {
  ...
  assert.doesNotMatch(router, /pandora/i);
});
...
test('native provider dispatcher has live GitHub Supabase Vercel and Google paths', () => {
  ...
  assert.doesNotMatch(native, /pandora/i);
});

test('native capability registry exposes honest availability without Pandora', () => {
  ...
  assert.doesNotMatch(registry, /pandora/i);
});
...
test('legacy provider-health storage is hidden behind Pandora-native view', () => {
  ...
  const refs = migration.match(/pandora_integration_health/gi) || [];
