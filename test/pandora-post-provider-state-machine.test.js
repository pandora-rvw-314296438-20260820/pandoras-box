...
test("ambiguous provider-supported replay reuses one immutable idempotency identity", async () => {
  ...
  const firstHandler = createPandoraMcpHandler(dependencies({ ledger: firstLedger, execute }));
  ...
  const secondHandler = createPandoraMcpHandler(dependencies({ ledger: secondLedger, execute }));
