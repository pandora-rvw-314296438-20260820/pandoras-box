...
exports.pandoraMcpVercelConfig = void 0;
exports.createPandoraMcpHandler = createPandoraMcpHandler;
...
} = require("./pandora-approval-authorization.js");
const {
...
const PANDORA_ACTION_SCOPES = Object.freeze([
    "pandora:read",
    "pandora:plan",
    "pandora:approve",
    "pandora:execute",
]);
const MCP_OAUTH_SCOPES = Object.freeze([...IDENTITY_SCOPES, ...PANDORA_ACTION_SCOPES]);
...
function assertToolScope(name, actor) {
    ...
    if (!required || (!granted.has(required) && !granted.has("pandora:*"))) {
        throw Object.assign(new Error(`OAuth scope ${required || "pandora"} is required`), {
            status: 403,
            requiredScope: required || "pandora",
        });
    }
}
    ...
    switch (name) {
        ...
        case "pandora_approve_plan": {
            ...
            const plan = assertPlanIdentity(await dependencies.ledger.approvePlan(
                ...
                planId,
                pandoraApproverAttribution(actor),
            ), planId, "approved");
...
function createPandoraMcpHandler(overrides = {}) {
    ...
    const coreHandler = async function pandoraMcpCoreHandler(request, response) {
    ...
    return function pandoraMcpHandler(request, response) {
...
async function handlePandoraMcp(request, response) {
    if (!defaultHandler) defaultHandler = createPandoraMcpHandler();
