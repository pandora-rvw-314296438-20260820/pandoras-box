...
function toVerificationRunRow(run, { verifierIdentity, builderIdentity = null, buildJobId = null } = {}) {
  ...
  return Object.freeze({ id: run.verification_run_id, organization_id: request.organization_id, project_id: request.project_id, project_spec_id: request.project_spec_id, project_version_id: request.project_version_id, build_job_id: buildJobId, source_kind: request.source_kind ?? (request.source_commit ? "git_commit" : "artifact_snapshot"), source_ref: request.source_ref ?? request.source_commit ?? request.project_version_id, source_commit: request.source_commit ?? null, source_digest: request.source_digest, artifact_digest: request.artifact_digest, migration_set_digest: request.migration_set_digest ?? null, runtime_target_digest: request.runtime_target_digest ?? null, preview_deployment_id: request.preview_deployment_id ?? null, target_environment: request.target_environment, required_check_profile: request.required_check_profile, requested_by: request.requested_by ?? null, builder_identity: builderIdentity, verifier_identity: verifierIdentity, identity_sha256: run.identity_digest, status: run.status, started_at: run.started_at, completed_at: run.completed_at, created_at: run.created_at });
}
