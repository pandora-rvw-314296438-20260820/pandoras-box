...
function operationArguments(toolName, args, authorization) {
  ...
  switch (toolName) {
    case 'request_build': return { ...lineage, requestedVersionId: args.version_id };
    case 'write_file': return { ...lineage, path: args.path, contentRef: args.content_ref };
    case 'delete_file': return { ...lineage, path: args.path };
    case 'move_file': return { ...lineage, fromPath: args.from_path, toPath: args.to_path };
    default: throw new Error(`Tool ${toolName} is not executable by Worker D`);
  }
}
...
function deriveAuthorization(proposal, policyDecision, context) {
  ...
  return Object.freeze({ actionHash, policyVersion: policyDecision.policy_version, reasonCode: policyDecision.reason_code ?? 'POLICY_ALLOWED' });
}
