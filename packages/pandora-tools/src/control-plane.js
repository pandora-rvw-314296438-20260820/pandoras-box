...
class ControlPlaneLineageSink { constructor(backend){this.backend=backend;this.durability="durable";} async record(event){if(event.kind==="policy_decision")await this.backend.recordPolicyAction(event);await this.backend.appendAudit(event);} }
