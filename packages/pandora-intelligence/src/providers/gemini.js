...
function normalizeHttpResponse(response) {
  ...
  const message = isRecord(body.error) && typeof body.error.message === 'string' ? body.error.message : `Gemini request failed (${status || 'unknown'})`;
  ...
  throw Object.assign(new Error(message), { code, retryable, retryAfterMs: Number(response.retryAfterMs ?? 0) || null });
}
...
function buildGeminiBody(request) {
  ...
  const budget = isRecord(request.budget) ? request.budget : {};
...
function extractCandidate(body) {
  ...
  const text = parts.filter(isRecord).map(part => typeof part.text === 'string' ? part.text : '').join('').trim();
