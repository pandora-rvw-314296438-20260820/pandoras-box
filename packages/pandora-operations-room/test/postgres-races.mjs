...
async function setup({budget=100,slots=8,org=null}={}) {
 ...
 await sql(`insert into public.organizations values(${literal(s.org)}) on conflict do nothing; insert into public.test_project_identity values(${literal(s.project)},${literal(s.org)}); select public.pandora_ops_initialize_v1(${args(s)},${budget},${slots}); select public.pandora_ops_register_worker_v1(${args(s)},'W1','fixture-builder-one',array['backend'],array[]::text[],4,'fixture:enrollment:one'); select public.pandora_ops_register_worker_v1(${args(s)},'W2','fixture-builder-two',array['backend'],array[]::text[],4,'fixture:enrollment:two'); select public.pandora_ops_control_v1(${args(s)},0,'resume');`);
 return s;
}
